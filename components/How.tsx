...
type HowTexts = {
  ...
  description: string;
  steps: {
    write: { title: string; text: string; badge: string };
    scan: { title: string; text: string; badge: string };
    organize: { title: string; text: string; badge: string };
  };
  features: {
    search: string;
    todos: string;
...
export default function How({ texts }: { texts: HowTexts }) {
  const steps = [
    {
      icon: <PencilLine className="h-5 w-5" />,
      title: texts.steps.write.title,
      text: texts.steps.write.text,
      badge: texts.steps.write.badge,
    },
    {
      icon: <Scan className="h-5 w-5" />,
      title: texts.steps.scan.title,
      text: texts.steps.scan.text,
      badge: texts.steps.scan.badge,
    },
    {
      icon: <FolderOpen className="h-5 w-5" />,
      title: texts.steps.organize.title,
      text: texts.steps.organize.text,
      badge: texts.steps.organize.badge,
    },
  ];
    ...
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 text-white">
        ...
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className={cx(inter.className, "mt-3 text-white/85 text-base")}>
            {texts.description}
          </p>
        </div>
        ...
        <div className="mt-12 md:mt-16 grid grid-cols-1 gap-8 lg:grid-cols-2 lg:gap-12">
          ...
          <div className="flex flex-col justify-center">
            <ul className="space-y-3 text-sm text-white/90">
              ...
              <li className="flex items-start gap-3">
                ...
                <span>{texts.features.todos}</span>
              </li>
              <li className="flex items-start gap-3">
                <CalendarDays className="mt-0.5 h-4 w-4 shrink-0" />
                <span>{texts.features.calendar}</span>
              </li>
              <li className="flex items-start gap-3">
                <Sparkles className="mt-0.5 h-4 w-4 shrink-0" />
                <span>{texts.features.tags}</span>
              </li>
            </ul>
          ...
          <div className="relative">
            ...
            {/* App Overlay */}
            <div
              className="w-[86%] max-w-sm ml-auto -mt-24 pr-1
            ...
            >
              <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/10 shadow-2xl backdrop-blur-md">
                <div className="flex items-center justify-between px-4 py-2 text-xs text-white/70">
                  <span className="truncate">{texts.mock.title}</span>
                  <span className="rounded bg-white/10 px-2 py-0.5">
                    {texts.mock.scanLabel}
                  </span>
                </div>
                <div className="bg-black/30 px-4 py-3">
                  <div className="mb-2 h-8 w-full rounded-md bg-white/10" />
                  <div className="space-y-2">
                    {texts.mock.lines.map((line, i) => (
                      <MockLine key={i} label={line} />
                    ))}
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2 px-4 py-3 text-xs">
                  {texts.mock.badges.map((b, i) => (
                    <Badge key={i}>{b}</Badge>
                  ))}
                </div>
              </div>
        ...
        <div className="mt-16 md:mt-20 grid grid-cols-1 gap-6 md:grid-cols-2">
          <ProblemSolutionCard
            variant="problem"
            title={texts.problems.problemTitle}
            points={texts.problems.problemPoints}
          />
          <ProblemSolutionCard
            variant="solution"
            title={texts.problems.solutionTitle}
            points={texts.problems.solutionPoints}
          />
        </div>
      </div>
    </section>
...
/* ---------- Steps ---------- */
function StepsRow({ steps }: { steps: any[] }) {
  return (
          ...
          >
            ...
            {i < steps.length - 1 && (
              <div className="flex justify-center md:justify-start">
                <ArrowRight className="my-2 h-6 w-6 text-white/40 md:my-0 md:mx-3 md:rotate-0 rotate-90" />
              </div>
            )}
          </div>
        ))}
